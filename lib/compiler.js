...
function compileCapture (argName, node) {
  var source = [];
  source.push("ret.push(" + argName + ");");
  source.push(compilePattern(argName, node.value));
  return source.join("\n");
}

function compileTypeName (argName, node) {
  return "if (!runt.matchesTypeName(" + argName + ", '" + node.value + "')) return false;"
}

function compileADT (argName, node) {
  var source = [];
  var clsArg = argName + "_class";
  source.push("var " + clsArg + " = runt.findADT(" + argName + ", '" + node.value + "');");
  source.push("if (!" + clsArg + ") return false;");
  source.push(
    "if (" + clsArg + ".__names.length !== " + node.children.length + ") ",
    "runt.errorADT('" + node.value + "', '" + node.pattern + "');"
  );

  for (var i = 0, argName2; i < node.children.length; i++) {
    argName2 = argName + "_" + i;
    source.push("var " + argName2 + " = " + argName + ".slot(" + i + ");");
    source.push(compilePattern(argName2, node.children[i]));
  }
  return source.join("\n");
}
